...
use super::targets::{
    ...
    install_devin, install_droid, install_dsh, install_grok, install_hermes, install_kilo,
    ...
    uninstall_cursor, uninstall_devin, uninstall_droid, uninstall_dsh, uninstall_grok,
...
fn install_target_inner(target: crate::api::schema::IntegrationTarget) -> io::Result<Vec<String>> {
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Dsh => {
            let installed = install_dsh()?;
            let mut messages = vec![format!(
                "installed dsh integration plugin to {}",
                installed.plugin_path.display()
            )];
            messages.push(if installed.updated_patch {
                format!(
                    "registered herdr dsh plugin entry in {}",
                    installed.patch_path.display()
                )
            } else {
                format!(
                    "herdr dsh plugin entry already current in {}",
                    installed.patch_path.display()
                )
            });
            messages
                .push("restart any running dsh session so it loads the herdr plugin".to_string());
            messages
        }
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Dsh => {
            let result = uninstall_dsh()?;
            let mut messages = Vec::new();
            if result.removed_patch_file {
                messages.push(format!(
                    "removed herdr-owned dsh patch layer at {}",
                    result.patch_path.display()
                ));
            } else if result.updated_patch {
                messages.push(format!(
                    "removed herdr dsh plugin entry from {}",
                    result.patch_path.display()
                ));
            } else {
                messages.push(format!(
                    "no herdr dsh plugin entry found in {}",
                    result.patch_path.display()
                ));
            }
            messages.push(if result.removed_plugin {
                format!(
                    "removed dsh integration plugin at {}",
                    result.plugin_path.display()
                )
            } else {
                format!(
                    "no dsh integration plugin found at {}",
                    result.plugin_path.display()
                )
            });
            messages
